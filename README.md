...
- [下载最新 FinderToolkit DMG](https://github.com/mora-na/FinderToolkit/releases/latest/download/FinderToolkit-latest.dmg)
- [查看最新 Release](https://github.com/mora-na/FinderToolkit/releases/latest)
...
README 使用固定名称的 `latest/download` 链接，因此后续正式 Release 会自动指向最新版本。脚本要求 Xcode 命令行工具和 `hdiutil`，不需要把证书或账号信息写入仓库。要制作可在 Gatekeeper 中直接通过的公开发行版，需要另外配置 Developer ID Application、Developer ID Installer（如有需要）和 Apple notarization 流程。
